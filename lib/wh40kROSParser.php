...
class wh40kROSParser extends wh40kParser {
    ...
    public function populateUnit($d, $clean) {
        ...
        if($d->selections->selection) {
            foreach($d->selections->selection as $dd) {
                if(
                    ((string) $dd['type'] == 'model') or 
                    (
                        ((string) $dd['type'] == 'upgrade') and 
                        (
                            ((string) $dd['name'] != 'Warlord') and 
                            (string) $dd->profiles->profile['typeName'] != 'Abilities' and 
                            (string) $dd->profiles->profile['typeName'] != 'Weapon'
                        )
                    ) or
                    ((string) $dd['type'] == 'unit')
                ) {
